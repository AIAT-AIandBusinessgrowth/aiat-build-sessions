...
| `dontAsk` | fragt **nie** nach — und **lehnt stattdessen jede Aktion ab**, die sonst eine Rückfrage ausgelöst hätte. Es laufen nur Lesezugriffe in den Arbeitsverzeichnissen, Befehle, die nichts verändern, vorab per Allow-Regel Erlaubtes und was ein Hook freigibt; abgelehnt werden u.a. Rückfragen an dich und Löschbefehle auf kritischen Pfaden. **Nicht zu verwechseln mit `bypassPermissions`:** `dontAsk` erlaubt **weniger** als Manual, nicht mehr | abgeriegelte CI-Läufe und Skripte mit gepflegter Allowlist — der Lauf wartet nie auf eine Eingabe ([Quelle](https://code.claude.com/docs/en/permission-modes), geprüft 2026-09-13) |
| `bypassPermissions` | **alle** Permission-Rückfragen **und** die Safety-Prüfungen aus: Aktionen laufen sofort durch, auch Schreibzugriffe auf geschützte Pfade. Das ist der Modus ohne Filter — nicht `dontAsk` | **nur in Container/VM** — niemals auf Maschinen mit echten Daten/Keys |

> **Frische-Hinweis (Stand 2026-07-30):** Auto Mode aktiviert man über **`--permission-mode auto`** bzw. den **Shift+Tab-Zyklus** — nicht über ein eigenes Aktivierungs-Flag. (Ein älteres `--enable-auto-mode`-Flag ist laut cli-reference **„Removed in v2.1.111"** — primärquellen-belegt, vom Changelog bestätigt.) Auf **Bedrock, Google Cloud's Agent Platform (früher Vertex) und Foundry** ist es **standardmäßig** im Shift+Tab-Zyklus verfügbar — die frühere Pflicht-Variable `CLAUDE_CODE_ENABLE_AUTO_MODE=1` (ab **v2.1.158 laut Doku**) ist **seit v2.1.207 wirkungslos** (No-op); unterstützt werden dort nur **Sonnet 5, Opus 4.7 or later und Fable 5**. **Plain-language:** Die obere Hälfte der Tabelle (Manual/`acceptEdits`/`plan`/`auto`) eskaliert *behutsam* — mehr Tempo, aber mit Filter. Die beiden letzten Modi ziehen dagegen in **entgegengesetzte** Richtungen: `dontAsk` fragt nie und **lehnt im Zweifel ab** — streng, aber nur mit gepflegter Allowlist brauchbar; `bypassPermissions` schaltet die Filter ab und gehört ausschließlich in eine wegwerfbare Sandbox/VM. Wer beide für dasselbe hält, verwechselt den strengsten Modus mit dem lockersten. *(Quelle: https://code.claude.com/docs/en/permission-modes · https://code.claude.com/docs/en/cli-reference · claude.com/blog/auto-mode, abgerufen 2026-07-30; Versionsangaben v2.1.111, v2.1.158 und v2.1.207 sowie die Beschreibungen von `dontAsk` und `bypassPermissions` geprüft 2026-09-13 gegen https://code.claude.com/docs/en/permission-modes)*
