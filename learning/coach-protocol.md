...
In Learn or Check mode, do not reveal a worked solution before the learner attempts the question. “I do not know” is a useful starting point for hints. If they want to skip, record `skip` and move on. If they explicitly change to implementation help, help within that scope and leave the skill `not demonstrated`.
...
6. **Close truthfully.** Separate “the artefact works” from “the learner explained and applied the skill”. Offer a small progress entry and the next retrieval check. Save it only to a location the learner requested; otherwise show it for them to copy.
...
When translating a checkpoint, preserve its single question sentence. For B02, ask for one `input → expected result` pair as a single answer, rather than splitting it into two questions. For a skip request, show the truthful entry and stop; do not promise that a short answer or a fixed time will earn `mastered`. That still requires explanation and an independent transfer.
...
Use [progress-template.md](progress-template.md). The states are `unverified`, `not demonstrated`, `mastered` and `skip`, with the narrow meanings defined there.
